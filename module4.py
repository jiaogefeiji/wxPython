...
class MPL_Panel_base(wx.Panel):
    ...
    def pyplotTrianglePoint(self, *args, **kwargs):
        ...
        self.plt.plot(triangle[0], triangle[1], 'k', lw=2,linestyle='-') 
        self.plot_ticks(left, right, bottom_tick, n) 
        self.plot_ticks(right, top, right_tick, n) 
        self.plot_ticks(left, top, left_tick, n) 

        #创建备注坐标
        self.plot_create_step(left, right, bottom_tick, n,'bottom')
        self.plot_create_step(right, top, right_tick, n,'right')
        self.plot_create_step(top,left , left_tick, n,'left')

        #收集不同边框的坐标数组
        bootomArr = self.plot_grid_line(left, right, bottom_tick, n,'bottom')
        rightArr = self.plot_grid_line(right, top, right_tick, n,'right')
        leftArr = self.plot_grid_line(left, top, left_tick, n,'left')


        # 创建 left right line 
        self.create_left_right_line(leftArr,rightArr)
        self.create_bootom_right_line(bootomArr,rightArr)
        self.create_bootom_left_line(bootomArr,leftArr)
        #去掉边框
        self.plt.axis('off')

        data = self.plt.scatter(x, y, marker=args[2],s=20,cmap='green', alpha=0.5)
        self.datas.append(data)

        self.plt.axis([left[0]-margin, right[0]+margin, left[1]-margin, top[1]+margin]) 
        self.plt.gca().set_aspect('equal', adjustable='box') 
        self.plt.legend(self.datas, self.names, loc=2)
        self.plt.show() 
    ...
    def create_left_right_line(self,leftArr,rightArr):
        for j in range(len(leftArr)):
            dataLeft = leftArr[j]
            dataRight = rightArr[j]
            self.plt.plot([dataLeft['x'],dataRight['x']],[dataLeft['y'],dataRight['y']])

    def create_bootom_right_line(self,bootomArr,rightArr):
        bootomArr.reverse()
    ...
    def create_bootom_left_line(self,bootomArr,rightArr):
        bootomArr.reverse()
        for j in range(len(bootomArr)):
            dataLeft = bootomArr[j]
            dataRight = rightArr[j]
    ...
    def plot_grid_line(self,start, stop, tick, n,type): 
        result = []
    ...
    def plot_create_step(self,start, stop, tick, n,type): 
        r = np.linspace(0, 1, n+1) 
        x = start[0] * (1 - r) + stop[0] * r 
        x = np.vstack((x, x + tick[0])) 
        y = start[1] * (1 - r) + stop[1] * r 
        ...
        for j in range(len(x[1])):
            ...
            if type=='left':
                ...
                sy = y[1][j]
            #备注开始
            self.plt.annotate(round(rticks[j], 2),
                            xy=(sx, sy),            # 在(3.3, 0)上做标注
                            fontsize=8,          # 设置字体大小为 16
                            xycoords='data')    # xycoords='data' 是说基于数据的值来选位置


    def plot_ticks(self,start, stop, tick, n): 
        r = np.linspace(0, 1, n+1) 
        x = start[0] * (1 - r) + stop[0] * r 
        x = np.vstack((x, x + tick[0])) 
        y = start[1] * (1 - r) + stop[1] * r 
        y = np.vstack((y, y + tick[1])) 
        self.plt.plot(x, y, 'k', lw=1) 
...
class MPL_Frame(wx.Frame):
    ...
    def __init__(self, title="地化制图V0", size=(900, 750)):
        ...
        self.Button2.Bind(wx.EVT_BUTTON, self.Button2Event)
          ...
          # 测试按钮2
        self.Button3 = wx.Button(self.RightPanel, -1, "三角", size=(100, 40), pos=(10, 10))
    ...
    def Button3Event(self, event):
        with wx.FileDialog(self, "Open XYZ file", wildcard="EXCEL files (*.xls;*.xlsx)|*.xls;*.xlsx",
            ...
            self.y = y;
            #self.MPL.cla()  # 必须清理图形,才能显示下一幅图
            self.MPL.pyplotTrianglePoint(x, y, 'v',filename)
